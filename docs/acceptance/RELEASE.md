...
| **v3.3.0** | **v2.4-F2 备份提醒**：距上次 zip 导出超 N 天（默认 7，1..365 可配，localStorage 持久化）→ 导出页 NAlert 横幅 + 侧栏 export 导航圆点（var(--accent)）；导出成功打戳后导出页横幅即时消除（侧栏圆点因 localStorage 非响应式，随刷新/项目切换消除）；watch(current.meta.id, immediate) 消 #/export 硬刷新竞态；不做自动化快照 | 单测 backupDue 5 例边界：从未备份 / 恰 N 天整触发 / 少一秒不触发 / 阈值≤0 恒提醒 / 戳损坏宁误报；竞态修复经任务评审 fix round 验证 |
