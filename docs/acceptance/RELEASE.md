...
- ✅ M4-F3 继承语义：fork 点及之前父线事件（继承样式区分），父线后续新增**不**进子线（fork.test 反复断言，含"父线新增后子线不可见"）
...
- ✅ M4-F8 横纵切换：默认 PC 横 / 移动窄屏纵；localStorage 持久化（swimlane）
- ✅ M4-E1 删线级联+主世界线不可删（按钮仅子线显示；integrity 测试级联含后代）
- ✅ M4-E2 fork 点被删/变草稿→标记失效可重指定不崩溃（fork.test 两种形态）
- ✅ M4-E3 草稿箱：未定时进顶部草稿箱、不出现在泳道；补时间选线入列；可放回（swimlane）
