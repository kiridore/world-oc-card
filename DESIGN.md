...
- 关系类型由用户自定义（亲属 / 敌对 / 恋人 / 从属 / 师徒…），带颜色与方向；
...
1. **时间轴视图（主视图）**：横向时间轴，事件按纪年 `time.value` 排列；世界线为水平轨道，fork 时新轨道纵向展开，fork 点画分叉曲线；每条线有颜色。自研 SVG 组件实现。
   - 交互：缩放 / 平移、点击事件打开编辑抽屉、轨道显隐切换、在轨道上任意时间点插入事件、在事件上"从此处创建 IF 线"。
