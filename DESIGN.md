...
1. **泳道视图（主 / 唯一视图）**：每条世界线一条泳道，事件为卡片。**横 / 纵双模式**——同一 DOM 由 CSS 控制流向（横向 = 泳道内卡片从左向右，PC 默认；纵向 = 泳道为列、卡片从上向下，移动端窄屏(<768px)默认，类 `git log --graph`）。用户可切换并持久化（localStorage `timelineOrientation`）。
   ...
   - fork：IF 线泳道从锚点事件岔出独立延伸，SVG 覆盖层画分叉连线（方向感知——横/纵两套坐标，共用同一“锚点事件 → 首事件”连线数据）。IF 线首卡**紧随锚点卡之后**（横：锚点右缘 + 卡间距；纵：锚点下缘 + 卡间距）——分叉线上首事件是分叉点事件的下一个事件，不与锚点同列对齐；锚点重排后首卡与曲线同步跟随。同一锚点多条 IF 线自然堆叠。
   - **背骨线**：同线相邻卡片间的间隙用泳道色实线段串联（各自边缘中点相连）；fork 曲线保持虚线；横向模式泳道面板统一横滚（v3.1.0）。
