...
> 版本：V1.4（2026-08-21）
> 变更：V1.4 回写 V1 实施期（v1.2–v2.3）的设计演进——角色新建即编辑、时间轴序位网格/聚簇/惯性/全部事件列表、关系箭头三态与曲线边、图谱缩放钳制与点阵背景、全应用动效细则（§2/§5.7），附录 A 补 A4–A9 实施决策。V1.3 新增 §5 视觉风格规范；百科条目名称改为项目内全局唯一（A1）。V1.2 新增通用模板系统。V1.1 角色表改为全自由字段块模型、存储改为文件夹式多 JSON 文件、图片独立存储、明确写策略。
> 状态：V1 已实现并发布（v1.0.0–v2.3.3），本文档与实现同步维护
...
1. **时间轴视图（主视图）**：横向轴为**事件序位轴**——全局（跨世界线）按绝对纪元排序后等距编序（dense rank，同刻共享序位），x 坐标只表达**先后顺序**，不按时间偏移量比例定位；事件点下方标注各自历法时间文本。世界线为水平轨道，fork 时新轨道纵向展开，fork 点画分叉曲线；每条线有颜色。自研 SVG 组件实现。
   - 交互：缩放 / 平移（作用于序位空间，缩放尺度有上下限，最远不超过全景外扩两格、最近不小于 0.05 序位跨度）、**平移带惯性**（松手按释放速度滑行，指数衰减 τ=180ms、速度上限 2.5px/ms，滚轮/再按下即打断；`prefers-reduced-motion` 跳过）、点击事件打开编辑抽屉、轨道显隐切换、在轨道上任意位置插入事件（时间取点击处两侧邻事件的中间值，保持先后）、在事件上"从此处创建 IF 线"；同刻多事件折叠为 ×N 聚合点，点击展开为纵向错开点，展开态提供"收起 ×N"入口。
   - 背景**序位网格竖线**：整数序位处淡竖线随缩放/平移/惯性同步移动，间距随缩放自适应抽稀（1/5/25…，保持 ≥14px 间距），暗示当前缩放比例。
   - 侧栏两层列表：**未定稿草稿**（无时间事件，可跳画布放置）与**全部事件列表**（按世界线分组、组内按绝对纪元升序，v1.3.0）——轨道上被遮挡/叠压的事件均可从这里可靠选中。
...
- 清单（v2.1.0 起）：页面切换入场（淡入上浮 8px，main 按路由重建仅做入场——规避多根视图离场动画不完成的问题）；首页项目卡片错落入场（40ms 递进封顶 200ms）；列表项浮现（角色/百科/事件/类型分组，搜索过滤时轻微浮现）；聚簇展开弹出（事件点 scale 0.6→1，transform-box fill-box）；弹窗/抽屉/下拉使用 Naive UI 内建过渡；
- **不做**装饰性循环动画；时间轴惯性滑动（§2.4.4）属操作物理反馈，不算装饰动效，同样尊重 reduced-motion。
